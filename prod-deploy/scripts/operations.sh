...
show_help() {
    ...
    echo "For infrastructure deployment, use ./scripts/deploy.sh instead."
    echo
    echo "=========================================="
    ...
    echo "  Note: HTTPS setup moved to ./scripts/deploy.sh wp-https-setup"
    echo
    echo "=========================================="
...
check_status() {
    ...
    if [ ! -d "$TERRAFORM_DIR" ]; then
        print_error "terraform/ directory not found. Please check your setup."
        exit 1
    fi
    
    # Check if Terraform is initialized
            if [ ! -f "$TERRAFORM_DIR/.terraform/terraform.tfstate" ]; then
        print_warning "Terraform not initialized. Run './scripts/deploy.sh init' first."
        return 1
    fi
