...
PROD_DEPLOY_DIR="$(dirname "$SCRIPT_DIR")"
TERRAFORM_DIR="$PROD_DEPLOY_DIR/terraform"
SCRIPTS_DIR="$SCRIPT_DIR"
PLANS_DIR="$PROD_DEPLOY_DIR/plans"
...
check_prerequisites() {
    ...
    if [ ! -d "$TERRAFORM_DIR" ]; then
        print_error "terraform/ directory not found. Please run this script from prod-deploy/scripts directory."
        exit 1
    fi
...
plan_deployment() {
    ...
    terraform plan -var-file="wordpress.tfvars" -out="$PLANS_DIR/infrastructure-plan.tfplan"
...
# Function to list available plans
list_plans() {
    print_header "Available Deployment Plans"
    
    if [ ! -d "$PLANS_DIR" ]; then
        print_warning "No plans directory found."
        return 0
    fi
    
    if [ -z "$(ls -A "$PLANS_DIR" 2>/dev/null)" ]; then
        print_warning "No plan files found in $PLANS_DIR"
        print_status "Run './deploy.sh plan' to create a new plan"
        return 0
    fi
    
    print_status "Available plans in $PLANS_DIR:"
    echo
    
    for plan_file in "$PLANS_DIR"/*.tfplan; do
        if [ -f "$plan_file" ]; then
            filename=$(basename "$plan_file")
            file_size=$(du -h "$plan_file" | cut -f1)
            file_date=$(stat -c %y "$plan_file" 2>/dev/null || stat -f %Sm "$plan_file" 2>/dev/null || echo "Unknown")
    ...
    print_status "To apply a specific plan:"
    echo "  ./deploy.sh apply ../plans/PLAN_NAME.tfplan"
    echo
    print_status "To create a new plan:"
    echo "  ./deploy.sh plan"
}
...
apply_deployment() {
    ...
    # If no plan file specified, try to find one
    if [ -z "$plan_file" ]; then
        # Check for default plan
        if [ -f "$PLANS_DIR/infrastructure-plan.tfplan" ]; then
            plan_file="$PLANS_DIR/infrastructure-plan.tfplan"
            print_status "Using default plan: $plan_file"
        else
            # List available plans and let user choose
            print_warning "No plan file specified and no default plan found."
            list_plans
            
            echo
            print_status "Please specify a plan file:"
            echo "  ./deploy.sh apply ../plans/PLAN_NAME.tfplan"
            echo
            print_status "Or create a new plan:"
            echo "  ./deploy.sh plan"
            exit 1
        fi
    fi
    ...
    if [ ! -f "$plan_file" ]; then
        ...
        if [ -f "$(basename "$plan_file")" ]; then
            plan_file="$(basename "$plan_file")"
        # Try relative to plans directory
        elif [ -f "$PLANS_DIR/$(basename "$plan_file")" ]; then
            plan_file="$PLANS_DIR/$(basename "$plan_file")"
        else
            print_error "Plan file not found: $plan_file"
            print_status "Available plans:"
            list_plans
            exit 1
        fi
    fi
    
    print_status "Using plan file: $plan_file"
...
show_help() {
    ...
    echo "  Docker Image: fiverivertutoring-wordpress:production"
    echo
    echo "  # Network:"
    ...
    echo "  plans              # List available deployment plans"
    ...
    echo "=========================================="
    echo "  apply [plan]       # Deploy infrastructure (uses default or specified plan)"
    ...
    echo "=========================================="
    echo "  wp-deploy          # Deploy WordPress application with Docker"
    ...
    echo "  wp-logs            # View WordPress application logs"
        ...
        echo "  Note: WordPress deployment uses direct Docker image transfer"
        echo
        echo "=========================================="
        echo "🗄️  DATABASE MIGRATION"
        echo "=========================================="
        echo "  db-migrate         # Migrate data from staging to production"
        echo "  database-migrate   # Alternative command for database migration"
        echo "  wp-db-migrate      # WordPress-specific database migration"
        echo "  migrate-data       # Simple migration command"
        echo "  Note: These commands run the migratedata-staging-to-prod.sh script"
        echo
        echo "=========================================="
    ...
    echo "  $0 apply           # Deploy all infrastructure (uses default plan)"
    echo "  $0 wp-deploy       # Deploy WordPress app with Docker"
    echo
    echo "  # Using existing plans:"
    echo "  $0 plans           # List available plans"
    echo "  $0 apply ../plans/infrastructure-plan.tfplan  # Use specific plan"
    echo
    ...
    echo "  $0 wp-start        # Start WordPress application"
    echo
    echo "  # Database management:"
    echo "  $0 db-manage copy-staging    # Copy staging data to production"
    echo "  $0 db-manage backup          # Create production backup"
    echo "  $0 db-manage verify          # Verify production database"
    echo "  $0 db-manage test-connection --debug  # Test with debug output"
    echo
    echo "  # HTTPS setup:"
...
generate_graph() {
    ...
    mkdir -p "$PROD_DEPLOY_DIR/graphs"
...
deploy_wordpress() {
    ...
    print_status "Running WordPress deployment with Docker..."
    "$SCRIPT_DIR/wordpress-management.sh" deploy
}

stop_wordpress() {
    print_header "Stopping WordPress"
    print_status "Running WordPress management script..."
    "$SCRIPT_DIR/wordpress-management.sh" stop
}

start_wordpress() {
    print_header "Starting WordPress"
    print_status "Running WordPress management script..."
    "$SCRIPT_DIR/wordpress-management.sh" start
}

view_wordpress_logs() {
    print_header "Viewing WordPress Logs"
    print_status "Running WordPress management script..."
    "$SCRIPT_DIR/wordpress-management.sh" logs
}

check_wordpress_status() {
    print_header "Checking WordPress Status"
    print_status "Running WordPress management script..."
    "$SCRIPT_DIR/wordpress-management.sh" status
}

backup_wordpress() {
    print_header "WordPress Backup"
    print_status "Running WordPress management script..."
    "$SCRIPT_DIR/wordpress-management.sh" backup
}
...
main() {
    case "${1:-help}" in
        ...
        apply)
            check_prerequisites
            apply_deployment "$2" # Pass the plan file argument
            ;;
        plans)
            list_plans
            ;;
        ...
        db-manage|database-manage|wp-db-manage)
            "$SCRIPT_DIR/migratedata-staging-to-prod.sh" "${@:2}"
            ;;
